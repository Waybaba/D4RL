...
class WaypointController(object):
    def __init__(self, maze_str, solve_thresh=0.1, p_gain=10.0, d_gain=-1.0): # ! origin solve_thresh=0.1
    ...
    # Only select one direction then move
    # def _new_target(self, start, target):
    #     ACT_NOOP = 0
    #     ACT_UP = 1
    #     ACT_DOWN = 2
    ...
    def _new_target(self, start, target):
        ...
        self.env.gs[target] = grid_spec.REWARD
        q_values = q_iteration.q_iteration(env=self.env, num_itrs=10, discount=0.99) # ! origin is 50 0.99
        # compute waypoints by performing a rollout in the grid
        max_ts = 100
        s = start_idx
        waypoints = []
        ...
        for i in range(max_ts):
            def softmax(x, temp):
                ...
                sm = (np.exp(x / temp) / np.float64(np.sum(np.exp(x / temp))))
                return sm
            # temp = 10. if i < 0.1 * max_ts else 0.1 # larger -> random
            # temp = max((1 - i/5) * 5.0, 0.1)
            temp = 0.1
            if temp is None: 
                # a = np.argmax(q_values[s])
                cur_xy = self.env.gs.idx_to_xy(s)
